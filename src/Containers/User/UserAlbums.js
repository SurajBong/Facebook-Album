  ...
  render() {
    return (
      <div className="container-fluid mt-3">
        <div className="row">
          {this.state.albumArray &&
            this.state.albumArray.map((albumItem, index) => {
              return (
                <AlbumComponent
                  key={index}
                  item={albumItem}
                  openAlbum={() => this.toggleAlbumContainer(albumItem.id)}
                />
              );
            })}
        </div>
...
const mapDispatchToProps = dispatch => {
  return {
    getAllPhotos: (albumId, auth) =>
      dispatch(userActions.getAlbumPhotos(albumId, auth))
  };
};
