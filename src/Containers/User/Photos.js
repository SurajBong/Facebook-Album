...
class Photos extends Component {
  ...
  toggleSlideShow = () => {
    this.props.history.push({
      pathname: this.props.match.url + "/slideshow",
      state: { photosSlideShow: this.state.photosStateArray }
    });
  };
  render() {
    return (
      <div className="container-fluid mt-3">
        <div className="row">
          {this.state.photosStateArray &&
            this.state.photosStateArray.map((photoItem, index) => {
              return (
                <AlbumPhotos
                  key={index}
                  item={photoItem}
                  openAlbumSlideShow={() => this.toggleSlideShow()}
                />
              );
            })}
        </div>
